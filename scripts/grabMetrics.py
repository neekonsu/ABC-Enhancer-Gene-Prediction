...
if __name__=="__main__":
    #cells = pd.read_csv("/mnt/lab_data3/kmualim/CorrelationPlotsCellLines/fullk562_data/Predictions_QCMetrics.txt", sep="\t", header=None)
    #files = sys.argv[1]
    #cells = pd.read_csv(files, sep="\t", header=None)
    #cells = pd.read_csv("SeventyOne_CellTypes.txt", sep="\t", header=None)
    #for i in ['tibial_nerve_female_adult']:
    for i in ['K562']:
        print(i)
...
#        prediction="/mnt/lab_data3/kmualim/PredictionFiles/Predictions_{}_qnorm/EnhancerPredictionsAllPutative.txt.gz".format(i)
